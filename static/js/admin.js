...
function setLoggedIn(loggedIn) {
  ...
  byId('dashboardSection').classList.toggle('hidden', !loggedIn);
  ...
  tbody.innerHTML = items.map((r) => {
    ...
    return `
      <tr>
        ...
        <td><span class="tag ${r.risk_level || 'low'}">${r.risk_level || 'low'}</span></td>
        <td title="${r.remark || ''}">${r.remark || ''}</td>
        <td>
          <button class="admin-btn ghost" onclick="openEditRecord(${r.id})">编辑</button>
          ${state.user?.role === 'admin' ? `<button class="admin-btn ghost" onclick="deleteRecord(${r.id})">删除</button>` : ''}
        </td>
      </tr>
    `;
  }).join('');
...
function setKpis(kpi = {}) {
  byId('kpiTotal').textContent = String(kpi.total || 0);
  byId('kpiRain').textContent = fmt(kpi.avg_rain);
  byId('kpiWater').textContent = fmt(kpi.avg_water);
  byId('kpiPeak').textContent = fmt(kpi.peak_water);
  byId('kpiEnergy').textContent = fmt(kpi.total_energy);
  byId('kpiOverflow').textContent = fmt(kpi.total_overflow);
}
...
function renderSummaryCharts(data = {}) {
  ...
  state.charts.summary.setOption({
    ...
    legend: { data: ['平均降雨', '平均水位', '总能耗', '总溢流'] },
    xAxis: { type: 'category', data: x },
    yAxis: [{ type: 'value', name: 'rain/water' }, { type: 'value', name: 'energy/overflow' }],
    series: [
      ...
      { name: '总能耗', type: 'bar', yAxisIndex: 1, data: rows.map((r) => Number(r.total_energy || 0)) },
  ...
  state.charts.mode.setOption({
    tooltip: { trigger: 'axis' },
    xAxis: { type: 'category', data: modeRows.map((r) => r.mode || '') },
    yAxis: { type: 'value' },
...
function clearRecordModal() {
  byId('recordId').value = '';
  ['mScenario', 'mRecordTime', 'mMode', 'mRain', 'mOverflow', 'mEnergy', 'mReward', 'mW1', 'mW2', 'mW3', 'mF1', 'mF2', 'mF3', 'mS1', 'mS2', 'mS3', 'mRemark'].forEach((id) => byId(id).value = '');
  byId('mRisk').value = '';
}
...
async function saveRecord() {
  ...
  const payload = {
    ...
    remark: byId('mRemark').value.trim(),
  };
...
async function loadUsers() {
  ...
  tbody.innerHTML = state.users.map((u) => `
    <tr>
      ...
      <td>${u.created_at || ''}</td>
      <td>
        <button class="admin-btn ghost" onclick="openEditUser(${u.id})">编辑</button>
        <button class="admin-btn ghost" onclick="deleteUser(${u.id})">删除</button>
      </td>
    </tr>
  `).join('');
}
...
async function bootstrapDashboard() {
  await Promise.all([loadRecords(), loadSummary(), loadUsers()]);
}

function resetFilters() {
  ['fScenario', 'fMode', 'fRiskLevel', 'fStartTime', 'fEndTime', 'fKeyword', 'fMinRain', 'fMaxRain', 'fMinOverflow', 'fMaxOverflow', 'fMinWater', 'fMaxWater'].forEach((id) => {
    byId(id).value = '';
  });
  state.page = 1;
}
...
function bindEvents() {
  ...
  byId('initBtn').addEventListener('click', doInit);
  ...
  byId('resetFilterBtn').addEventListener('click', async () => {
    resetFilters();
    await Promise.all([loadRecords(), loadSummary()]);
  });
  ...
  byId('newRecordBtn').addEventListener('click', () => {
    clearRecordModal();
    openRecordModal(false);
  });

  byId('cancelRecordBtn').addEventListener('click', closeRecordModal);
  ...
  byId('cancelUserBtn').addEventListener('click', closeUserModal);
  byId('saveUserBtn').addEventListener('click', saveUser);
